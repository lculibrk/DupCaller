...
def genotypeSSSnv(
    seq_mat, qual_mat, prior_mat, params, A_log_prob, T_log_prob, C_log_prob, G_log_prob
):
    np.seterr(all="raise")
                        ...
                        if (
                            ...
                            try:
                                ...
                                print(
                                    readSet[0].reference_start + 100000,
                                    region_end,
                                    readSet[0].reference_end,
                                    contig_end,
                                    reference_mat_start,
                                    chromNow,
                                    rec.reference_name,
                                    readSet[0].reference_name,
                                )
                        ...
                        mut = {
                            ...
                            "infos": {
                                ...
                                "TC": ",".join(
                                    [
                                        str(_)
                                        for _ in F1R2_count[:, muts_ind[nn]].tolist()
                                    ]
                                ),
                                "BC": ",".join(
                                    [
                                        str(_)
                                        for _ in F2R1_count[:, muts_ind[nn]].tolist()
                                    ]
                                ),
                            },
                            "formats": ["AC", "RC", "DP"],
