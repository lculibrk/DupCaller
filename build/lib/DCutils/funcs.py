...
def detectMismatches(seqs,reference_int,chrom,start,end,params):
    ...
    ref_qual = total_count_mat[reference_int, np.ogrid[:n]] 
    alt_qual = total_count_mat[alt_int, np.ogrid[:n]] 
    antimask[(total_count_mat >= 1).sum(axis=0) > 1] = False
    antimask[ref_qual<=60] = False
    antimask[np.logical_and(alt_qual<=60,alt_qual!=0)]  = False
    antimask[ref_int == 4] = False
