...
if __name__ == "__main__":
    ...
    with open(params["output"] + "/summary.txt", "w") as f:
    ...
    with open(params["output"] + "/duplex_group_stats.txt", "w") as f:
        f.write(f"duplex_group_strand_composition\tduplex_group_number\tcoverage\tmutation_count\n")
        muts_by_duplex_group = OrderedDict()
        for read_num in duplex_read_num.keys():
            muts_by_duplex_group[read_num] = 0
        for mut in mutsAll:
            TC_total = sum(mutsAll["infos"]["TC"])
            BC_total = sum(mutsAll["infos"]["BC"])
    ...
    muts_by_group = np.loadtxt(params["output"] + "/duplex_group_stats.txt",skiprows=1,dtype=float,delimiter='\t',usecols=(2,3)).transpose()
    burden_lstsq,_,_,_ = np.linalg.lstsq(muts_by_group[0,:],muts_by_group[1,:])
    bootstrap_lstsqs = []
    for _ in range(10000):
        muts_by_group_resampled = np.random.Generator.choice(muts_by_group,muts_by_group.size[1],axis=1)
        burden_lstsq_resampled,_,_,_ = np.linalg.lstsq(muts_by_group_resampled[0,:],muts_by_group_resampled[1,:])
        bootstrap_lstsqs.append(burden_lstsq_resampled)
    bootstrap_lstsqs = sorted(bootstrap_lstsqs)
    ...
    plt.savefig(params["output"] + "/burden_by_duplex_group_size.png")
